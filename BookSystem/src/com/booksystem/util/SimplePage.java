 ...
 * 分页对象<br>
 * 需要设置 : <br>
		simplePage.setPage(每页显示几条数据);<br>
		simplePage.setPageNo(当前第几页);<br>
    	simplePage.setAll(总数据);<br>
    	注:setAll()需要在最后执行，不然没有效果
...
public class  SimplePage<T>  implements Serializable{
    ...
    /**
              * 设置所有的数据
     * */
    public void setAll(List<T> allList) {
    	if(allList!=null) {
    		setPageDataNum(allList.size());
