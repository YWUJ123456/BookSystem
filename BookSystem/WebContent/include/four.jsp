...
<%-- 
	...
	6,在div的合适位置添加 分页插件 html代码 : 
		eg: 
		<div class="divtable">
		    <c:forEach items="${simplePage.list}" var="stu">
				${stu.id }-${stu.name }<br>
			</c:forEach>
			<!-- 分页插件 -->
			<div class="p_btn">
...
后台:
	普通servlet:
		protected void doGet(HttpServletRequest request, HttpServletResponse response)
			...
			//数据库中所有的学生对象
			List<Student> stus = 查询数据库得到的所有学生对象;
			
			SimplePage<Student> simplePage = new SimplePage<>();
			
			
			simplePage.setPage(2);
			simplePage.setPageNo(pageNo);
			simplePage.setAll(stus);
...
<c:if test="${simplePage.pageCount -1 >= 0  }">
	...
	<div class="page-nav">
		...
		<input type="text" name="pageNo"  style="width: 40px;">
 		<span onclick="gotoPage()"><button>转到</button></span>
	</div>
</c:if>
<script type="text/javascript">
/**
